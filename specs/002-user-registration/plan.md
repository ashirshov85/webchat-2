...
**Performance Goals** (спека): письмо ≤ 2 мин в 95% (SC-001); ≥ 95% корректных логинов без заметной задержки, прозрачный refresh (SC-003); 10+ неудачных входов/мин на аккаунт → эффективное ограничение подбора (SC-004); доступность публичных endpoints при отказе email-сервиса, 100% сбоев доставки в observability (SC-007); контролируемая деградация под профилем нагрузки (SC-008). Расчёт соответствия бюджетам конституции (280 rps логинов, ~100k Redis GET/s на denylist при 100k msg/s) — [research.md §13](./research.md).
