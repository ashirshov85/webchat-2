...
- [ ] T025b [P] [US1] Заменить в `frontend/src/api/client.ts` (строки 10–12) пару `import type` + `export type` на единственную форму `export type { components, paths } from './schema'` (SonarQube `typescript:S7763`, issues №3 и №4 — оба экземпляра правила на одной строке 12, закрываются одним атомарным изменением); инварианты: type-only re-export без runtime-эмита ([api-contract.md §4](./contracts/api-contract.md)), поверхность экспорта `components`/`paths` неизменна, `contracts/openapi.yaml` и `frontend/src/api/schema.d.ts` не трогаются (drift-check конвейера 001 остаётся зелёным); критерий: `pnpm lint && pnpm typecheck && pnpm test` exit 0, оба issues `typescript:S7763` закрыты после перескана SonarQube
