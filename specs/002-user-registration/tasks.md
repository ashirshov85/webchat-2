...
- [ ] T002 [P] Создать `deploy/local/docker-compose.yml`: postgres:17 (:5432), redis:7 (:6379), mailpit (:8025 UI / :1025 SMTP) ([quickstart.md §2](./quickstart.md)); критерий: `docker compose -f deploy/local/docker-compose.yml up -d` → все контейнеры запущены, Mailpit UI доступен на :8025 (quickstart.md §2)
- [ ] T003 [P] Создать dev-манифесты Kustomize: `deploy/k8s/overlays/dev/postgres.yaml` (Deployment replicas:1 strategy:Recreate + PVC 5Gi RWO + Secret), `deploy/k8s/overlays/dev/redis.yaml` (Recreate + PVC 1Gi, `--maxmemory 256mb --maxmemory-policy volatile-ttl`, appendonly off), `deploy/k8s/overlays/dev/mailpit.yaml` (Deployment + Service) ([research.md §10](./research.md)); критерий: `kubectl kustomize deploy/k8s/overlays/dev` рендерится без ошибок
