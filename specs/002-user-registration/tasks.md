...
- [X] T025a [P] [US1] Включить Gradle dependency verification (SonarQube `kotlin:S6474`, VULNERABILITY — конституция V «безопасность по умолчанию»): сгенерировать контрольные суммы зависимостей `./gradlew --write-verification-metadata sha256` в `backend/`, закоммитить `backend/gradle/verification-metadata.xml`; при любом последующем изменении зависимостей (`backend/build.gradle.kts`, `backend/gradle/libs.versions.toml`) — регенерировать metadata в той же задаче; критерий: `./gradlew check` exit 0 с чистого клона с включённой верификацией, порча checksum в metadata → сборка падает (fail-safe), issue `kotlin:S6474` закрыт после перескана SonarQube
- [X] T025b [P] [US1] Заменить в `frontend/src/api/client.ts` (строки 10–12) пару `import type` + `export type` на единственную форму `export type { components, paths } from './schema'` (SonarQube `typescript:S7763`, issues №3 и №4 — оба экземпляра правила на одной строке 12, закрываются одним атомарным изменением); инварианты: type-only re-export без runtime-эмита ([api-contract.md §4](./contracts/api-contract.md)), поверхность экспорта `components`/`paths` неизменна, `contracts/openapi.yaml` и `frontend/src/api/schema.d.ts` не трогаются (drift-check конвейера 001 остаётся зелёным); критерий: `pnpm lint && pnpm typecheck && pnpm test` exit 0, оба issues `typescript:S7763` закрыты после перескана SonarQube
- [X] T025c [P] [US1] Пометить порт `backend/src/main/kotlin/webchat/backend/email/EmailGateway.kt:29` как `fun interface EmailGateway` (SonarQube `kotlin:S6517`, MAJOR): изменение — одно ключевое слово `fun`; интерфейс сохраняется как именованный DIP-порт с KDoc-контрактом (конституция VIII: домен зависит от абстракций; throw-семантика ошибок доставки — SC-007, запрет логирования тел писем — SC-005), реализация `SmtpEmailGateway` (T018) и внедрение в `OutboxPoller` (T021) не меняются; бонус — SAM-конверсия для тест-дублей (`EmailGateway { ... }` в T050); замена на `typealias (EmailMessage) -> Unit` отклонена: теряет именованный порт и потребовала бы правки Spring-связывания; критерий: `./gradlew check` (вкл. ktlint/detekt) exit 0, поведение IT `RegistrationFlowIT` без изменений, issue `kotlin:S6517` закрыт после перескана SonarQube
- [X] T025d [P] [US1] Расширить секцию `spring.mail` в `backend/src/main/resources/application.yml`: `username: ${SPRING_MAIL_USERNAME:}`, `password: ${SPRING_MAIL_PASSWORD:}` и `properties.mail.smtp.auth ${SPRING_MAIL_SMTP_AUTH:false}` + TLS-флаги через env (`mail.smtp.ssl.enable ${SPRING_MAIL_SMTP_SSL_ENABLE:false}` для 465, `mail.smtp.starttls.enable ${SPRING_MAIL_SMTP_STARTTLS_ENABLE:false}` для 587) — все дефолты false/пустые, локальный Mailpit (T018, quickstart §2) работает без изменений; реальный SMTP-провайдер подключается чисто env-конфигурацией без правки кода (`SmtpEmailGateway`/spring-boot-starter-mail, DIP-порт не меняется); секреты только env/K8s Secrets (конституция V, SC-005); критерий: `./gradlew check` exit 0, `ContextLoadsIT` (T012) зелёный без mail-env и с выставленными `SPRING_MAIL_*` (auth=true, ssl=true)
- [X] T025e [US1] Перевести dev-стенд на реальный SMTP (после T025d): `deploy/k8s/overlays/dev/backend-auth-env.yaml` — `SPRING_MAIL_HOST=smtp.mail.ru`, `SPRING_MAIL_PORT=465`, `SPRING_MAIL_USERNAME`/`SPRING_MAIL_PASSWORD` из `secretKeyRef` Secret `smtp-credentials` (создаётся вне репо на стенде: `kubectl -n dev create secret generic smtp-credentials --from-literal=...`; технический ящик + «пароль для внешних приложений»), `SPRING_MAIL_SMTP_AUTH=true`, `SPRING_MAIL_SMTP_SSL_ENABLE=true`, `AUTH_MAIL_FROM=<технический ящик>` (From обязан совпадать с ящиком); в той же задаче добавить отложенный `APP_PUBLIC_BASE_URL=https://dev.webchat.lkshr.ru` — ссылки писем ведут на публичный SPA вместо дефолта `http://localhost:5173` (дефект обнаружен на стенде: письма доставляются в Mailpit с localhost-ссылками); локальный docker-compose остаётся на Mailpit (quickstart §2); критерий: `kubectl kustomize deploy/k8s/overlays/dev` рендерится, после `kubectl apply -k` + `rollout status deploy/backend` письмо регистрации уходит провайдеру (outbox `sent`), ссылка в письме — `https://dev.webchat.lkshr.ru/confirm-registration?token=...`
- [X] T025f [US1] Ручная валидация полного цикла на dev-стенде с реальной доставкой (после T025e; quickstart §4.1–4.2 на реальном email): `POST /api/v1/auth/register` на `https://dev.webchat.lkshr.ru` → письмо на реальный ящик → переход по ссылке → confirm → set-password → аккаунт `active` в PG; отказоустойчивость: неверный/отозванный app-пароль → письмо зависает `pending` в outbox с retry, публичные endpoints без 5xx (SC-007); откатываемость: возврат mail-env на `mailpit.dev.svc.cluster.local:1025` + снятие auth-флагов восстанавливает quickstart-сценарий через Mailpit без пересборки образа; критерий: конец-в-конец регистрация подтверждена на стенде, откат проверен
...
- [X] T028 [P] [US2] Реализовать `backend/src/main/kotlin/webchat/backend/auth/security/JwtService.kt`: подпись/проверка ES256 (P-256), claims `sub`/`sid`/`jti`/`iat`/`exp`/`typ:"access"`, заголовок `kid`, ключи из `AUTH_JWT_KEYS` (kid → PEM) + `AUTH_JWT_ACTIVE_KID`, TTL 5 мин (конфиг.) — [research.md §4](./research.md); критерий: GREEN в составе T026
...
- [X] T054 [P] Линт и форматирование: `./gradlew ktlintCheck detekt` (workdir `backend/`), `pnpm --dir frontend lint`; критерий: обе команды exit 0
- [X] T055 Аудит секретов (SC-005): gitleaks по репозиторию; в логах/БД/выгрузках нет открытых паролей и токенов (только Argon2-хеши и SHA-256); креды только env/K8s Secrets
- [X] T056 Контрактный конвейер и CI зелёные: `vacuum lint -e contracts/openapi.yaml`, drift-check TS-типов (regen + `git diff --exit-code`), `oasdiff breaking` — только additive 0.2.0; полный прогон `./gradlew test` и `pnpm --dir frontend test`
- [X] T057a [P] Создать нагрузочный smoke-сценарий `load/k6/auth.smoke.js`: burst `login` ≈280 rps (вращающиеся identifier'ы) + burst `register` 50 rps с одного источника — профиль [research.md §13](./research.md); k6 thresholds: 0×5xx, `http_req_duration` p(95)<500 мс для валидных логинов под лимитом (coarse-guard: прогон ко-локален, сеть ≈ loopback), `/actuator/health` 200 весь прогон, 429 с `Retry-After` при превышении лимитов, валидный трафик под лимитом обслуживается (SC-008, конституция VI)
- [X] T057b Прогнать сценарий по [quickstart.md §4.7](./quickstart.md) локально против docker-compose + финальная проверка после прогона — `GET /actuator/metrics/http.server.requests?tag=uri:/api/v1/auth/login` → percentile 0.95 ≤ 0.5 s (точное соответствие SC-003: серверная задержка без сети клиента) (зависит от T057a)
