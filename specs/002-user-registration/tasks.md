...
- [ ] T009 Создать `backend/src/main/kotlin/webchat/backend/config/SecurityConfig.kt`: `SecurityFilterChain` stateless, csrf off, permitAll ТОЛЬКО `/api/v1/auth/register/**`, `/api/v1/auth/login`, `/api/v1/auth/refresh`, `/api/v1/auth/password-reset/**` + технические endpoints фичи 001: `/actuator/health/**` (пробы K8s liveness/readiness), `/actuator/prometheus` (scrape), `/actuator/metrics/**` (проверка SC-003 в T057; вне публичного API-контракта — [technical-endpoints.md 001 §1](../../001-project-foundation/contracts/technical-endpoints.md)); `anyRequest().authenticated()`; единый 401 `application/problem+json` (`authenticationEntryPoint`); бин `PasswordEncoder` — Argon2id m=19456,t=2,p=1 в `DelegatingPasswordEncoder` ([research.md §3, §5](./research.md); [api-contract.md §3](./contracts/api-contract.md))
...
- [ ] T026 [P] [US2] Написать IT `backend/src/test/kotlin/webchat/backend/auth/SessionIT.kt`: US2-1…US2-5 — вход по username и по email → 200 пара; единый 401 (тайминги выравнены — фиктивный хеш); 403 «завершите регистрацию»; ротация (старый refresh повторно → 401, цепочка отозвана, denylist sid); logout → 204, оба токена отклоняются; две параллельные сессии независимы — logout одной не отзывает другую (edge spec); в auth_events зафиксированы `logout` и reuse-отзыв (без секретов) — FR-013 ([api-contract.md №5–7](./contracts/api-contract.md))
