...
2. 10 неудачных `POST /auth/login` подряд по одному аккаунту в течение минуты: видны
   возрастающие задержки ответов (с 5-й попытки — после 4 накопленных неудач,
   [research.md §8](./research.md)); 11-я попытка — **429** с `Retry-After` ≈ остаток
   15 мин; после истечения блокировки корректный пароль входит сразу.
