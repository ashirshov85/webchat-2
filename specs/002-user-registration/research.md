...
**до** проверки пароля. Для `/auth/login` проверка счётчика выполняется ПЕРВОЙ, до бакетов
...
| login: лимиты | 10/мин на IP; 10/мин на аккаунт; приоритет маршрута: счётчик `login:fail` → бакет аккаунта → бакет IP; замедление с 4-й неудачи; блокировка с 10-й на 15 мин | FR-009, SC-004 |
