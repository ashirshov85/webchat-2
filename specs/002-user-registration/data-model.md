...
- Повторная регистрация: частичное пересечение (занят только username или только email) с аккаунтом
  **любого статуса** → 409 с указанием поля, без раскрытия статуса держателя (US1-4); полное совпадение
  пары (username+email) на **незавершённом** аккаунте → возобновление: аннулируются старые токены,
