...
- [ ] T026 [US2] `outbox.ts`: константа лимита 1000 записей на пользователя; enqueue с вытеснением старейших `sending` (начало массива): `state='failed'`, `errorCode='queue_overflow'`, автоповторы прекращаются, событие уведомления для баннера; `failed`-записи не вытесняются — в `frontend/src/chats/outbox.ts`; приёмка: T024 зелёный
