...
- [x] T037 [US3] Валидация истории: прогон `UnreadConvergenceIT` + frontend-тестов T033 зелёными, ручная проверка quickstart §3.4 ( realtime/sync-рост, частичное постраничное чтение, оффлайн-прочтения с перезапуском, ✓✓ за оффлайн, заморозка при блокировке) — файлы по месту реализации
