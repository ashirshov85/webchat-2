...
6. Параллельная отправка: Bob offline с непустой очередью, Alice пишет в тот же чат;
   сеть Bob включается → очередь уходит, сообщения Bob/Alice чередуются по порядку
   принятия сервером (seq), дублей и потерь нет (edge «одновременная отправка»).
