...
### S9. Экран входа — визуальный прототип (US7, P2; дополнение 2026-09-20)
...
`index.css` не тронут. Экран регистрации — US8/Phase 11 (шаги добавляются
после T071; до него `/register` в старом стиле без глобального заголовка —
временное состояние T067).
...
6. h1 скрыт: глобальный заголовок «WebChat» отсутствует на `/login` (и на
   `/register` — временное состояние до US8, см. выше); на прочих
   маршрутах (например, `/forgot-password`) — присутствует.
7. `prefers-reduced-motion: reduce` (DevTools → Rendering → emulate CSS
   ...
   (18s linear infinite).
8. Регресс окружения: страницы вне scope (register, confirm-registration,
...
**Откат**: revert Phase 10 (tasks.md T063–T069) — поведение входа не
зависело от редизайна.
