...
Презентационный редизайн `/login` и `/register` по дизайн-прототипу
(research §22, §22.6): «латунная» карточка на полноэкранном фоне, шрифты
Cinzel/EB Garamond (self-host `@fontsource`), шестерёнка-бейдж, заклёпки,
поля с иконками, глаз показа пароля, SSO-кнопки «Продолжить с …» с брендовыми
иконками. Контракт и поведение входа/регистрации не меняются (FR-018/FR-022).
...
2. Для SSO-части — как S1 (локально: `deploy/local` + dex, или dev-стенд);
   без провайдеров сценарий проверяется на парольной форме (S9 шаг 2).

**Ручная проверка**:

1. Открыть `/login`: фон на весь экран с затемнением, карточка по центру
   (380px), рамка/тени/заклёпки, STEAMCHAT Cinzel + подпись
   CONNECT·CHAT·EXPLORE, шестерёнка-бейдж вращается (при включённом
   `prefers-reduced-motion` — статична); глобальный заголовок «WebChat»
   отсутствует и здесь, и на `/register`; на прочих маршрутах (например,
   `/forgot-password`) — присутствует.
2. Парольная форма: placeholder «Email или имя пользователя»/«Пароль»,
   иконки в полях, глаз переключает тип поля и aria-label; неверные
   креды → ошибка в карточке (role=alert), верные → welcome; «ВОЙТИ»
   блокируется на отправке.
3. SSO-блок (при включённых провайдерах): разделитель «ИЛИ», кнопки
   «Продолжить с …» с иконками (google/github/yandex/vk — брендовые, dex —
   generic) в порядке конфигурации; клик → переход на authorize (как S1);
   при пустом перечне блок и разделитель скрыты.
4. Ссылка «Зарегистрироваться» → `/register` — карточка того же стиля
   (фон, бейдж, бренд, заклёпки, шрифты), заголовок «Создание аккаунта»,
   поля «Имя пользователя»/«Email» с иконками, кнопка
   «ЗАРЕГИСТРИРОВАТЬСЯ», футер «Уже есть аккаунт? Войти» → `/login`;
   divider/SSO-блока нет.
5. Регистрация: отправка валидных данных → success-сообщение с email
   (role=status); занятый username → 409-коды в карточке (role=alert);
   кнопка блокируется на отправке.
6. Страницы вне scope (confirm-registration, set-password,
   forgot/reset, sso/callback) — без визуальных изменений (стили scoped
   в `auth-theme.css`, `index.css` не менялся).
7. Узкий вьюпорт (mobile): карточки ≤90vw, прокрутка при малой высоте,
   бейдж не обрезается.

**Откат**: revert фаз Phase 10–11 (tasks.md T063–T073) — поведение
входа/регистрации не зависело от редизайна.

**Приёмка**: раздел прошёл ручную проверку (`pnpm --dir frontend dev`
или dev-стенд); фиксация в PR.
