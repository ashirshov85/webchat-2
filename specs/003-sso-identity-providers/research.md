...
## 22. Экраны входа и регистрации: дизайн-прототип — нормативный инвентарь US7/US8 (дополнение 2026-09-20)

**Decision**: источник визуала — прототип `tmp/login.html` (не версионируется);
инвентарь ниже нормативен для реализации, отклонения фиксируются здесь.
Регистрация (US8) прототипа не имеет — строится на том же инвентаре (§22.6).

### 22.1 Дизайн-токены

| Токен | Значение |
|---|---|
| `--gold` | `#c9a24a` (базовое золото, иконки полей) |
| `--gold-light` | `#e8c877` (заголовок, фокус-рамка, ссылки) |
| `--gold-dark` | `#8a6a2c` (рамки, разделитель) |
| `--panel-bg` | `rgba(15, 13, 10, 0.82)` (карточка) + `backdrop-filter: blur(2px)` |
| `--input-bg` | `rgba(25, 21, 16, 0.85)` (поля, SSO-кнопки) |
| Текст полей / placeholder | `#eee2c8` / `#8d7f63` |
| Подпись бренда / футер | `#b6a26a` / `#a9986e` |
| Шрифты | заголовки/кнопка: Cinzel 700–800 (латиница, letter-spacing 2–3px); текст: EB Garamond 400 (кириллица включена в @fontsource) |
| Фон страницы | `public/bg2.png`, `center/cover fixed` + радиальное затемнение `rgba(0,0,0,0.15)→0.55` |

### 22.2 Инвентарь компонентов (селекторы прототипа → классы реализации)

- **Карточка** `.auth-card` — 380px (max 90vw), border 2px `--gold-dark`,
  radius 12px, тройная тень (внешняя + inset), padding `46px 34px 32px`.
- **Заклёпки** ×4 — 10px круги, radial-gradient золото, в 10px от углов,
  `aria-hidden`.
- **Шестерёнка-бейдж** — 76px круг над карточкой (top −38px), radial-градиент
  тёмный + border 3px `--gold`; внутри: вращающийся cog-SVG (18s linear,
  `prefers-reduced-motion: reduce` → пауза) + статичный чат-пузырь SVG.
- **Бренд** — `STEAMCHAT` (Cinzel 800, 30px) + `CONNECT·CHAT·EXPLORE`
  (12px, uppercase, letter-spacing 3px).
- **Поля** — padding `13px 14px 13px 42px`, иконка слева 13px (юзер/замок,
  stroke `currentColor`), глаз справа (кнопка, переключение type пароля);
  focus: border `--gold-light` + ring `rgba(232,200,119,0.25)`.
- **Кнопка «ВОЙТИ»** — золотой градиент 180° (`gold-light→gold→gold-dark`),
  текст `#241b0c`, Cinzel 700, letter-spacing 2px; hover brightness 1.08,
  active translateY(1px); disabled — текущая логика `submitting`.
- **Разделитель** «ИЛИ» — flex-линии с градиентом в прозрачность; скрывается
  вместе с SSO-блоком при пустом перечне провайдеров.
- **SSO-кнопки** — во всю ширину, иконка 18px + «Продолжить с {displayName}»;
  hover: border `--gold-light`, фон светлее.
- **Футер** — «Нет аккаунта?» + ссылка «Зарегистрироваться» → `/register`.
- **Ошибки/успех** — `role=alert` / `output` внутри карточки, тёмно-полупрозрачные
  панели в красной/зелёно-золотой гамме (существующее поведение текстов
  problem+json сохраняется).

### 22.3 Мапа иконок провайдеров

| `provider.id` | Иконка |
|---|---|
| `google` | многоцветный логотип (SVG из прототипа) |
| `github` | силуэт кота, `fill #e8dcc0` (SVG из прототипа) |
| `yandex` | красный скруглённый квадрат `#FC3F1D` + белый «Я» |
| `vk` | синий скруглённый квадрат `#0077FF` + белые «VK» |
| `dex` и прочие | generic: золотой ключ/globe stroke-иконка в палитре карточки |

Иконка чисто презентационная (`aria-hidden`), доступное имя кнопки — текст
«Продолжить с {displayName}». Состав кнопок определяется только API — иконка
не влияет на перечень (US7-4). «Microsoft» из прототипа не подключается
(нет такого провайдера в конфигурации).

### 22.4 Интеграция в SPA (отклонения от прототипа — осознанные)

- **Fullscreen при `#root{max-width:480px}`**: обёртка `.auth-screen` —
  `position: fixed; inset: 0` с фоном и затемнением; карточка —
  `margin: auto` (scroll-safe центрирование: при малой высоте вьюпорта
  страница прокручивается, бейдж не обрезается). Прототип кладёт фон на
  `body` — в SPA это задевало бы остальные маршруты.
- **Глобальные стили**: `index.css` красит `section/button/input` глобально —
  новые правила специфичнее (`.auth-card input` и т.д.), глобальный файл
  не правится (FR-021); overrides: `button{align-self}` → `stretch`/`auto`,
  `form div{flex}` → `block`.
- **A11y**: в прототипе поля без `<label>` — реализация сохраняет видимый
  placeholder и добавляет sr-only label (тесты `getByLabelText` остаются
  валидными), `autoComplete` не меняется.
- **Русский текст**: тексты прототипа нормативны; доступные имена кнопок
  обновляются в тестах синхронно (`Login`→`ВОЙТИ`, `Google`→`Продолжить с
  Google`, `Or continue with`→разделитель «ИЛИ»).
- **Шрифты**: `@fontsource/cinzel` (700, 800) + `@fontsource/eb-garamond`
  (400) — импорт весов в `LoginPage.tsx`; CDN-вариант прототипа отвергнут
  (офлайн-стенды, приватность, детерминированная сборка). Вес 600 прототипа
  не используется фактическими правилами — не подключается (YAGNI).
- **Бейдж-шестерёнка и заклёпки** — декоративные div/svg с `aria-hidden`.

**Alternatives considered**: CSS-модули — отвергнуты (проект на одном
глобальном CSS, одна страница не повод менять конвенцию); tailwind/UI-кит —
отвергнут (YAGNI, прототип — чистый CSS); перенос стилей в `index.css` —
отвергнут (утечка золотой темы на остальные страницы).

### 22.5 Тестирование US7/US8

Vitest/RTL (jsdom — визуал не проверяется, только DOM-контракт):
обновлённые `LoginPage.test.tsx` (русские label'ы, кнопка «ВОЙТИ», alert/status,
глаз переключает type и aria-label) и `LoginPageSso.test.tsx` (кнопки
«Продолжить с …», скрытие блока при пустом перечне, redirect, returnTo);
Register-ветка `RegisterPages.test.tsx` (label «Имя пользователя»/«Email»,
кнопка «ЗАРЕГИСТРИРОВАТЬСЯ»; ConfirmRegistration/SetPassword-ветки файла не
правятся — страницы вне scope). Визуальная часть — ручная проверка quickstart
S9 (оба экрана). Снапшот-тесты не вводятся (хрупкость к косметике; тесты
поведения, а не пикселей — конституция VI).

### 22.6 Экран регистрации (US8): состав и переиспользование

**Decision**: `/register` использует тот же визуальный язык US7 без
отдельного прототипа; стили — общий файл `frontend/src/auth/pages/auth-theme.css`
(классы `.auth-*`), импортируется обеими страницами; CSS не дублируется.

Состав карточки регистрации:

- Та же композиция: fixed-обёртка `.auth-screen` (фон + затемнение), карточка
  `.auth-card` с заклёпками, шестерёнка-бейдж, бренд-блок `STEAMCHAT` +
  `CONNECT·CHAT·EXPLORE` (узнаваемость пары экранов).
- Заголовок формы: `Создание аккаунта` (EB Garamond, `--gold-light`,
  по центру, на месте brand-sub-отступа login-карточки).
- Поля: «Имя пользователя» (иконка юзера) и «Email» (иконка конверта/@) —
  то же оформление, что в §22.2 (placeholder + sr-only label, `autoComplete`);
  глаз не нужен (пароля нет — фича 002 задаёт его по ссылке из письма).
- Кнопка «ЗАРЕГИСТРИРОВАТЬСЯ» — тот же золотой градиент/шрифт, что «ВОЙТИ»;
  disabled = submitting.
- Divider «ИЛИ» и SSO-кнопки отсутствуют: регистрация через IdP формой не
  выполняется — первый вход через провайдера создаёт аккаунт сам (US2);
  отсутствие блоков вёрстку не ломает (flex-композиция карточки).
- Футер: «Уже есть аккаунт? **Войти**» → `/login` (зеркало футера login
  «Нет аккаунта? **Зарегистрироваться**» → `/register`).
- Сообщения: `role=alert` (ошибки 400/409 — коды `problem+json` через
  существующий `problemMessage`, без перевода кодов) и `role=status` (успех —
  русская формулировка «Проверьте свою почту ({email}): мы отправили ссылку
  для подтверждения, она действует 24 часа»); токены карточки §22.2.

**Alternatives considered**: отдельный `RegisterPage.css` — отвергнут
(дублирование карточки/палитры, рассинхрон при правках темы); редизайн всего
флоу регистрации (confirm/set-password) — отвергнут вне scope (YAGNI, фаза
задаёт пароль один раз по ссылке).

## Сводка дополнения 2026-09-20

| Вопрос | Решение |
|---|---|
| Источник визуала | §22.1–22.2: токены и инвентарь прототипа, нормативны |
| Иконки провайдеров | §22.3: мапа по `provider.id` + generic fallback |
| Fullscreen vs #root 480px | §22.4: fixed-обёртка + scroll-safe `margin:auto` |
| Шрифты | §22.4: @fontsource self-host, CDN отвергнут |
| Тесты | §22.5: DOM-контракт в Vitest, визуал — quickstart S9 |
| Экран регистрации | §22.6: общий `auth-theme.css`, та же карточка, состав полей/футера регистрации; divider/SSO отсутствуют |
