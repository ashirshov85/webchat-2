...
**Performance Goals**:_budgetы конституции II (1M CCU); SSO-вход добавляет 2 HTTP-редиректа и ≤2 вызова IdP (внешняя задержка вне нашего бюджета); таймауты к IdP (connect+read) — 5 с на вызов (SC-005: понятная ошибка ≤5 с); деградация одного провайдера не влияет на парольный вход и остальных провайдеров (изоляция по построению — нет общего состояния)
