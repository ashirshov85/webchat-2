...
в 100% прогонов (SC-003); последние сообщения p95 ≤ 3 с после открытия, страница истории p95 ≤ 3 с
...
«Performance & Data Retention Standards») — системная cross-cutting-задача, вынесена
в ROADMAP (пункт Т4, фича 009; триггер: archival MUST быть введён до появления
сообщений старше 12 месяцев); в рамках 004 история хранится в PG без ограничения —
обоснование: расчёт объёмов ([research.md §12](./research.md)) укладывается в
вертикальный PG на горизонте фичи, SQL-паттерны приложения не меняются при вводе
archival позже. Нагрузочная валидация бюджетов принципа II (1M CCU / 100k msg/s) в 004 —
расчётная ([research.md §12](./research.md)): k6 smoke (T066) проверяет целевые
p95-латентности и отсутствие 5xx при 429 на минимальном профиле VU-пар, но не
воспроизводит пиковые объёмы; полномасштабные нагрузочные тесты бюджетов — платформенная
...
единственная проекция). quickstart.md проверяем без ручных правок артефактов. Observability: распределённый tracing
наследуется от инфраструктуры 001–003 (micrometer-tracing + OTel OTLP,
`backend/build.gradle.kts`); авто-инструментация покрывает новые REST-endpoints и SSE-маршрут,
отдельных задач не требуется — верификация спанов включена в аудит T068. Выводы таблицы
