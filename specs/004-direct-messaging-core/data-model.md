...
- Чат исключён из списка «Чаты» ⟺ `hidden AND last_seq <= deleted_up_to_seq` (удалён без
  новых сообщений, FR-021). Чат с видимыми сообщениями (`last_seq > deleted_up_to_seq`)
  сортируется по последнему видимому сообщению; пустой чат (в т.ч. созданный ensure-ом,
  FR-018) показывается ниже переписок (FR-014); новый входящий (`seq` выше водяного знака)
  снова показывает чат, история не восстанавливается.
...
[клиент] отправляется ──201/200──► [сервер] доставлено (✓) ──peer.last_read_seq ≥ seq──► прочитано (✓✓)
     ...
     └─ 403 you_are_blocked / chat_blocked_by_you / 409 / 400: ──► не отправлено (терминальный; ручной повтор тем же id
...
- **ChatView** (GET /chats/{id}, ensure): `chatId, peer, blockedByMe, peerReadUpToSeq,
  myReadUpToSeq` (поле `hidden` в API не экспонируется: удаление выражается исключением
  из списка №12 и пустой историей по водяному знаку).
