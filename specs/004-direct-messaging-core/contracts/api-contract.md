...
на пользователя**; лимит поиска пользователей — **30 запросов/мин на пользователя** (FR-016).
...
| # | Метод и путь | Тело (request) | Успех | Ошибки |
|---|---|---|---|---|
| 19 | `GET /users/search?query=<string>` | — | `200` `{users: [PublicUser]}` — точное совпадение полного email ИЛИ полного логина, без учёта регистра (FR-016): `query` содержит `@` → сравнение `lower(email)`, иначе `lower(username)` (username не допускает `@` — правило 002); 0..1 элемент (пустой результат — корректный, edge) | `400` `query_missing` (\|query\| < 1 или > 254); `401`; `429` `flood_limit` + `Retry-After` (FR-016: не более 30 запросов/мин на пользователя, защита от перебора — сек до доступного токена) |
